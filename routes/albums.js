 ...
 */
router.post('/', (req, res) => {
    const { access_hash, download_count, download_limit, folder_id } = req.body;
    db.run(`INSERT INTO Albums (access_hash, download_count, download_limit, folder_id) VALUES (?, ?, ?, ?)`,
        [access_hash, download_count, download_limit, folder_id],
        function (err) {
...
router.put('/:id', (req, res) => {
    const { access_hash, download_count, download_limit, folder_id } = req.body;
    db.run(`UPDATE Albums SET access_hash = ?, download_count = ?, download_limit = ?, folder_id = ? WHERE id = ?`,
        [access_hash, download_count, download_limit, folder_id, req.params.id],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
            if (this.changes === 0) return res.status(404).json({ message: 'Album not found' });
